...
{
    class MediumSizeButtonViewModel : TopMenuButtonViewModel
    {

      //Calls the constructor of TopMenuButtonViewModel
        public MediumSizeButtonViewModel(Bitmap image)
            : base(image)
        { }


        //Click event for medium size button
        public override void ClickExecuted()
        {
            ...
            string query = "insert into Ordered_items(Item_name,Item_image,Item_size) select Item_name, Item_image,'Medium' from Selected_items where Id=(select max(Id) from Selected_items)";
